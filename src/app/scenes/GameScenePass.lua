...
function GameScene:createLayer()
    ...
    --返回
    local backbtn = ccui.Button:create():loadTextures("backup.png", "backdown.png", ""):pos(70,size.height-90):addTo(layer,0)
    backbtn:addTouchEventListener( function(sender, eventType)
        if eventType == ccui.TouchEventType.ended then
            scheduler.unscheduleGlobal(self.clock)
            onBackClick()
        end
    end )
...
function GameScene:createWidgets(layer)
    ...
    local function refreshClock(dt)
        self.clocknumber = self.clocknumber+1
        clock:setString(self.clocknumber)
    end
    
    self.clock = scheduler.scheduleGlobal(refreshClock,1.0)
...
function GameScene:contactAtPos()
    ...
    if not emptybubb then
        ...
        GameManager.currpasstime = self.clocknumber
        scheduler.unscheduleGlobal(self.clock)
    ...
    GameManager:collectBubbsColor()
    self.bodypos = nil
    print(2,"碰撞处理完毕,self.bodypos = %s",self.bodypos)

    local collect = {}
    for i,v in ipairs(GameManager:getStageInfo()) do
        for m,n in ipairs(v) do
            if n.color ~= 0 then
                table.insert( collect, n.color )
            end
        end
    end

    if #collect == 0 then
        GameManager.failreason  = 4
        if #GameManager.levelinfo < GameManager:getGameStats() then
            table.insert(GameManager.levelinfo,{pass = 1,time = self.clocknumber})
        else
            GameManager.levelinfo[GameManager:getGameStats()].pass = 1
             local mintime = self.clocknumber < GameManager.levelinfo[GameManager:getGameStats()].time and self.clocknumber or GameManager.levelinfo[GameManager:getGameStats()].time
            GameManager.levelinfo[GameManager:getGameStats()].time = mintime
        end
        GameManager.currpasstime = self.clocknumber
        GameManager.currpasslevel = true
        scheduler.unscheduleGlobal(self.clock)
        local scene = require("app.scenes.OverScene")
        cc.Director:getInstance():replaceScene(scene.create())
    end
