...
function GameManager:getStageSize()
    if not stageSize then
        ...
        local top = CONFIG_SCREEN_HEIGHT- 160
...
function GameManager:newStage()
    ...
    elseif stageType ==2 then
        ...
        print(1,"stage type error in function new stage")
    end
...
-- 收集舞台上带颜色的泡泡
function GameManager:collectBubbsColor()
    self.filtcolor = {}
    local allcolors = {}
    for row,rowdata in ipairs(stageInfo) do
        -- dump(rowdata)
        for col,bubbinfo in ipairs(rowdata) do
            if bubbinfo.color ~= 0 then
                table.insert(allcolors, bubbinfo.color)
            end
        end
    end
    table.insert(allcolors, stageInfo["randombubb"].color)
    table.insert(allcolors, stageInfo["shootbubb"].color)
    table.sort(allcolors)
    for i=1,#allcolors do
        if allcolors[i] ~= allcolors[i+1] then
            self.filtcolor[#self.filtcolor+1] = allcolors[i]
        end
    end
    local bubbleNum = {}
    for i,v in ipairs(self.filtcolor) do
        bubbleNum[v] = BUBBIMG[v]
    end
    BUBBIMG = bubbleNum

    dump(self.filtcolor,"过滤后的颜色666")
end
...
function GameManager:combineParent(father_id1,father_id2)
    for i,sontag in ipairs(parentTable[father_id2]) do
        ...
        if not parentTable[father_id1] then
            print(2,"father_id1 = %s , father_id2 = %s ",father_id1,father_id2)
            dump(parentTable)
        end
...
function GameManager:spawnRandomBubb()
    if not stageInfo.randombubb then
        ...
        if gameStats ~= 7 and #self.filtcolor ~= 0 then
            local rancolor = self.filtcolor
            local num = math.random(#rancolor)
            bubbinfo.color = self.filtcolor[num]
        else
            bubbinfo.color = math.random(#BUBBIMG)
        end
        -- bubbinfo.posx = winsize.width/2 -  2*BUBBSIZE
        -- bubbinfo.posy = stageSize[2]-BUBBPOSY
        bubbinfo.posx = winsize.width/2 - BUBBPOSY +370
        bubbinfo.posy = stageSize[2]-BUBBPOSY-620
        stageInfo.randombubb = bubbinfo
    else
        if gameStats ~= 7 then
            local rancolor = self.filtcolor
            local num = math.random(#rancolor)
            stageInfo.randombubb.color = self.filtcolor[num]
        else
            stageInfo.randombubb.color = math.random(#BUBBIMG)
        end
    end
end
...
function GameManager:spawnShootBubb()
    ...
    bubbinfo.posx = winsize.width/2 - 70
    bubbinfo.posy = stageSize[2]-BUBBPOSY+10
...
function GameManager:refreshShootBubb()
    if gameStats ~= 7 and #self.filtcolor ~= 0  then
        local rancolor = self.filtcolor
        local num = math.random(#rancolor)
        stageInfo.shootbubb.color = self.filtcolor[num]
    else
...
function GameManager:refreshStageInfoOnMoveDown()
    ...
    local newrow = {}
    -- 更好的随机泡泡生成
    local grop = math.random(1,3)
    if gameStats ~= 7 and #self.filtcolor ~= 0 then
        local rancolor = self.filtcolor
        local num = math.random(#rancolor)
        local clrindex = self.filtcolor[num]
    else
        local clrindex = math.random(#BUBBIMG)
    end
    -- local clrindex = math.random(#BUBBIMG)
    for i = 1, MAXODD do
        if i % grop == 0 then 
            if gameStats ~= 7 and #self.filtcolor ~= 0 then
                local rancolor = self.filtcolor
                local num = math.random(#rancolor)
                clrindex = self.filtcolor[num]
            else
                clrindex = math.random(#BUBBIMG)
            end
            -- clrindex = math.random(#BUBBIMG)
        end 
